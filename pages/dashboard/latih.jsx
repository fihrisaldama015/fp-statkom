...
const Latih = () => {
  const [dataLatih, setDataLatih] = useState();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await axios.get("/api/latih");
        setDataLatih(data);
        setIsLoading(false);
      } catch (error) {
        console.log(error);
  ...
  if (!login)
    return (
      <div className="flex flex-col gap-3 h-full items-center justify-center">
        <h1>Login required to Open Data Latih</h1>
  ...
  return (
    <div className="my-16 px-6 flex flex-col w-full h-full justify-center gap-6">
      <h1 className="text-2xl font-bold text-center">Data Latih</h1>
